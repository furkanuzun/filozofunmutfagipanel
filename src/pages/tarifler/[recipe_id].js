...
const TarifEkle = () => {
  ...
  const getRecipe = () => {
    setIsFetching(true);
    axios
      .get(`http://localhost:3000/api/recipes/recipe?_id=${router.query.recipe_id}`)
      .then((res) => {
        const { recipe } = res.data;
        const { malzemeler, talimatlar, fotograf, kategoriler, kalori_sayaci } = recipe;
        getCats(kategoriler, kalori_sayaci);
        setTarif(recipe);
        setIngredients(malzemeler);
        setInstructions(talimatlar);
        setPhoto(fotograf);
      })
      ...
      axios
        ...
        .then((res) => {
          ...
          kategoriler.forEach((kategori) => {
            selectedCats.push(
              res.data.categories.find((category) => category._id === kategori._id)
            );
          });
          setSelectedMainCategories(selectedCats);
          resolve();
        })
        .catch((err) => reject())
        .finally((res) =>
      ...
      axios
        ...
        .then((res) => {
          ...
          let selectedMeter = res.data.calorieMeters.find(
            (meter) => meter._id === kalori_sayaci._id
          );
          setSelectedCalorieMeters(selectedMeter);
          resolve();
        })
        .catch((err) => reject())
    );
  ...
  const [selectedCalorieMeters, setSelectedCalorieMeters] = React.useState({});
  ...
  const handleSubmit = (e) => {
    ...
    const url = "http://localhost:3000/api/recipes/edit"; // Uygulamanın port numarasını uygun şekilde değiştirin
    ...
    toast.promise(postRecipe, {
      ...
      success: {
        render({ data }) {
          ...
          return `Tarif düzenlendi!`;
        },
        autoClose: 2000,
      },
      error: "Tarifi düzenlerken bir hatayla karşılaştık :(",
    });
  };
  return (
    <div className="p-4">
            ...
            <CardContent sx={{ px: 3 }}>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2 grid grid-cols-4 gap-4">
                  ...
                  <div className="col-span-1">
                    <TextField
                      value={tarif.besin_degerleri.karbonhidrat}
                      onChange={(e) =>
                        setTarif((prevState) => ({
                          ...prevState,
                          besin_degerleri: {
                  ...
                  <div className="col-span-1">
                    <TextField
                      value={tarif.besin_degerleri.protein}
                      onChange={(e) =>
                        setTarif((prevState) => ({
                          ...prevState,
                          besin_degerleri: {
                  ...
                  <div className="col-span-1">
                    <TextField
                      value={tarif.besin_degerleri.yag}
                      onChange={(e) =>
                        setTarif((prevState) => ({
                          ...prevState,
                          besin_degerleri: {
                          ...
                          renderValue={(selected) => {
                            return (
                              <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
                                {selected.map((value) => (
                                  <Chip key={value._id} label={value.kategori_adi} />
                                ))}
                              </Box>
                            );
                          }}
                          ...
                          {calorieMeters.map((name) => {
                            return (
                              <MenuItem
                                key={name._id}
                                value={name}
                                // style={getStyles(name, selectedCalorieMeters, theme)}
                              >
